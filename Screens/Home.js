import { View, Text, StatusBar, SafeAreaView, Image, TextInput, ScrollView } from "react-native";
  ...
  return (
    <SafeAreaView style={tw`mt-5 bg-white pt-5 `}>
      ...
      <View style={tw`flex-row items-center  mt-2 px-3 pb-2 `}>
        <View style={tw`flex-row  items-center flex-1  bg-gray-300 mr-1 p-2 rounded`}>
          <ServerIcon size={25} style={tw`text-green-500 mr-2 `}></ServerIcon>
          <TextInput style={tw``} placeholder="Search Food"></TextInput>
        </View>
        <View>
          <AdjustmentsVerticalIcon size={25} style={tw`bg-gray-300`} ></AdjustmentsVerticalIcon>
        </View>
      </View>

     {/* Scrollable View (body) */}
   
   <ScrollView style={tw`bg-gray-100 mt-4` }
